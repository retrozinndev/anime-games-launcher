...
fn crc() -> std::io::Result<()> {
    ...
    test(
        HashAlgorithm::Crc64Ecma182,
        &[1, 2, 3],
        &[54, 144, 58, 165, 77, 32, 242, 166],
        &[81, 123, 3, 180, 238, 229, 68, 45]
    )?;

    test(
        HashAlgorithm::Crc64Xz,
        &[1, 2, 3],
        &[44, 123, 100, 92, 240, 51, 6, 40],
        &[214, 140, 244, 224, 137, 158, 146, 218]
    )?;
...
fn seahash() -> std::io::Result<()> {
    test(
        ...
        &[1, 2, 3],
        &[253, 6, 208, 102, 41, 87, 194, 46],
        &[198, 124, 217, 159, 132, 16, 68, 15]
    )
}
...
fn siphash() -> std::io::Result<()> {
    test(
        ...
        &[1, 2, 3],
        &[147, 39, 201, 56, 128, 161, 114, 41],
        &[0, 81, 84, 23, 45, 86, 65, 28]
    )?;
    ...
    test(
        ...
        &[1, 2, 3],
        &[31, 39, 10, 239, 52, 109, 73, 38],
        &[253, 21, 148, 136, 45, 29, 30, 84]
    )?;
...
fn xxh() -> std::io::Result<()> {
    ...
    test(
        ...
        &[1, 2, 3],
        &[127, 228, 15, 8, 248, 172, 154, 196],
        &[56, 174, 103, 233, 202, 247, 105, 152]
    )?;

    test(
        HashAlgorithm::Xxh3_64,
        &[1, 2, 3],
        &[170, 2, 102, 97, 95, 93, 65, 96],
        &[38, 80, 40, 142, 134, 52, 27, 1]
    )?;

    test(
        HashAlgorithm::Xxh3_128,
        &[1, 2, 3],
        &[149, 83, 215, 44, 132, 3, 219, 119, 80, 221, 71, 68, 132, 242, 29, 83],
        &[8, 227, 171, 120, 172, 217, 7, 95, 163, 213, 121, 211, 165, 118, 132, 118]
    )?;
